...
fetch('https://api.themoviedb.org/3/movie/top_rated?language=ko&page=1', options)
    ...
    .then(data => {
        ...
        // id랑 class를 동시에 가져도 문제 될 것이 없기때문에 그냥 class를 추가한다.
        // fetch 작업이 아니라 css 작업을 위해서 추가한 것이다.
        container.classList.add('movieData')
...
function handleSearch(event) {
    ...
    if (typeBTN === '결과 내 검색') {
        ...
        if (searchTerm !== '') { // 검색어가 없는 경우에는 작동하지 않는다.
            let container = document.getElementById('movieData');
            let cards = container.getElementsByClassName('card');
            console.log(cards)
            Array.from(cards).forEach(function (card) {

                let filteredCards = Array.from(cards).filter(function (filtering) { // filter 메서드에 보내는 콜백함수
                    let filterdTitle = filtering.textContent.toLowerCase();
                    return filterdTitle.includes(searchTerm);
                });

                if (filteredCards.includes(card) === false) {
                    card.style.display = 'none';
                }

                
                // includes를 사용하면 array를 새로 만들지 않고, filter, reduce, map, find를 사용하면 array를 만들기 때문에,
                // includes가 더 경제적이라고 판단된다. 하지만 과제에서 요구한 조건에 includes가 없어서 filter로 구현했다. 
               
                // let title = card.textContent.toLowerCase(); //영어 검색의 경우 대소문자 구분을 없애기 위해 전부 소문자 처리한다.
              
                // 검색결과에 따라서 스타일을 추가한다.
                // if (title.includes(searchTerm) === false) {
                //     card.style.display = 'none';
                //      //일치하지 않는 카드를 가리게 설정 // card.style.display = 'block'; //일치하는 카드를 보여주게 설정
                // }
            });
        }
    } else {
